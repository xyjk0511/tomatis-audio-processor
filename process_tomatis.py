...
def process(
    ...
    fc=1000.0,
    slope=12.0,
    c1_low=+5.0, c1_high=-5.0,
    c2_low=-5.0, c2_high=+5.0,
    up_delay_ms=250.0,
    n_fft=4096,
    hop=2048,
    state_csv_path=None,
    ...
    with sf.SoundFile(in_path, 'r') as fin:
        ...
        try:
            ...
            out_is_flac = False
            print(f"✓ 输出格式: WAV 24-bit (稍后需转换为 FLAC)")
        print()
        ...
        g1 = db_to_lin(g1_db)
        g2 = db_to_lin(g2_db)
        print(f"✓ C1 增益曲线: {len(g1)} 频率 bins")
        print(f"✓ C2 增益曲线: {len(g2)} 频率 bins")
        print()
        ...
        while True:
            ...
            while True:
                ...
                # 能量平均（power average）: sqrt(mean(L^2 + R^2) / 2)
                # 不是波形平均 (L+R)/2，后者在反相时会严重低估能量
...
def main():
    ...
    ap.add_argument("--c1_low", type=float, default=5.0, help="C1 低频增益（dB）")
    ap.add_argument("--c1_high", type=float, default=-5.0, help="C1 高频增益（dB）")
    ap.add_argument("--c2_low", type=float, default=-5.0, help="C2 低频增益（dB）")
    ap.add_argument("--c2_high", type=float, default=5.0, help="C2 高频增益（dB）")
