...
def build_eq_from_residual_safe(freqs_rfft, res_freq, res_db,
...
def main():
    ...
    ap.add_argument("--out_audio", required=True)
    ap.add_argument("--diff_csv", default="diff_spectrum.csv")
    ...
    ap.add_argument("--clamp_hi", type=float, default=1.0)
    ap.add_argument("--hf_start", type=float, default=3000.0)
    args = ap.parse_args()
    ...
    eq_lin, eq_db = build_eq_from_residual_safe(
        freqs, res_freq, res_db_s,
        clamp_lo=-1.0, clamp_hi=args.clamp_hi,
    ...
    print(f"[DONE] Applied SafeB residual EQ to {args.out_audio}")
